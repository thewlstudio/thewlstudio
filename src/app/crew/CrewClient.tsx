...
export default function CrewClient({ members }: { members: CrewMember[] }) {
    return (
        <section className="pt-40 pb-24 px-4 lg:px-12 mx-auto max-w-[90rem]">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-y-10 gap-x-4 md:gap-y-16 md:gap-x-6 mt-8">
                {members.map((member, index) => (
                    ...
                    >
                        ...
                        <Link href={`/crew/${member.slug}`} className="mb-2 md:mb-3 font-black text-lg md:text-2xl tracking-tighter transition-colors duration-300 group-hover:text-black">
                            {member.name}
                        </Link>
                        ...
                        <div className="relative aspect-[3/4] w-full bg-neutral-100 overflow-hidden shadow-lg">
                            ...
                            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-between p-6 pointer-events-none">
                                ...
                                <div className="w-full text-right h-8 flex items-start justify-end">
                                    <span className="text-[8px] md:text-[10px] text-white/50 tracking-widest font-bold uppercase">{member.role}</span>
                                </div>

                                {/* Center Text (Hover State Name) */}
                                <div className="w-full flex flex-col items-center justify-center px-2">
                                    <h3 className="text-white text-sm md:text-lg lg:text-2xl font-black tracking-tighter uppercase whitespace-nowrap drop-shadow-lg italic">
